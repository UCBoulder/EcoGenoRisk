...
def tax_clustering(ec_space, synbio):
    ...
    print("Enter the known taxonomy of your synbio organism. If no data is available, list Synbio as an entry for all ranks: ")
...
def calculating_distance(input_df, genome_names, lineage_preference, index_names_of_cluster, genome_ID, rank):
    ...
    distances_parallel.to_csv('Overall_distance_matrix.txt', header=False, index=False, sep='\t')
    if lineage_preference == "Y":
