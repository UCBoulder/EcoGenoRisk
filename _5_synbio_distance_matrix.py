...
def read_in_binary_matrix(name, dest):
    ...
    binary_matrix_table = pd.read_csv(dest+'/'+name , delimiter=" ", header = 0, index_col = 0)
...
## Outputs include preference on weighting (Y/N) and the name of the sheet used for scaling
    ...
    if preference=="Y":
        ...
        if personal == "Y":
            ...
            print("You have selected to use the default filter: /home/anna/PycharmProjects/pythonProject"
                  "/2022_5_24_Proposed_Filter.csv")
            filter_used = "/home/anna/PycharmProjects/pythonProject/2022_5_24_Proposed_Filter.csv"
            default  = 0
        else:
...
def tax_clustering(ec_space, synbio):
    ...
    full_lineage =  pd.read_csv("/home/anna/PycharmProjects/pythonProject/taxonomy.tsv", delimiter  = '\t', header = 0)
    ...
    if lineage_preference == "Y":
        ...
        ec_grouped = ec_space_lin.groupby(cluster_rank, as_index=True)[ec_col].mean()
    ...
    #np.savetxt("ec_space_"+cluster_rank.lower()+".txt", ec_grouped)
    return ec_grouped, lineage_preference, index_names_of_cluster, cluster_rank
...
def calculating_distance(input_df, genome_names, lineage_preference, index_names_of_cluster):
    ...
    if lineage_preference=="Y":
        #If the user chose to cluster, distance matrix will have the rank-related index names for synbio
         distances_synbio = pd.concat([index_names_of_cluster.reset_index(drop=True),
                                       pd.DataFrame(distances_parallel).reset_index(drop=True)], axis= 1)
    else:
    ...
    return distances_synbio.iloc[:,0:2]
...
def pass_to_distance(big_matrix_name, genome_ID, location):
    ...
    np.savetxt(name, synbio_clustered_distances, fmt='%s')
