...
def diamond_impl(dest):
    ...
    if os.path.exists(
            new_destination + '/reference.dmnd'):  # if there currently is no reference library (.dmnd), makedb creates a DIAMOND library
   ...
   # for item in domain_folder, if item is a faa file, complete diamond analysis
    for item in sorted(os.listdir(dest)): #save the sorted list of genome file names for traceback 
        ...
        if item.endswith('.faa'):
            ...
            print(matches)
            # If genome has already undergone DIAMOND search, a blank will be printed, otherwise the subprocess function
            # will run the diamond search
            if os.path.exists(new_destination+"/"+matches):
                ...
                blastp = ['diamond', 'blastp', '-d', new_destination + '/reference.dmnd', '-q', file_path, '-o', matches,
