...
binary_matrix <-  read.table("complete_binary_matrix.csv", header=TRUE, quote = "", sep = '\t')
# Output from python script 5, contains the EC space after binning depending on which rank the user selected
ec_rank <-read.delim('ec_space_class.txt', header = FALSE, quote = "",  sep= '\t')
# Distance matrix from script 5, used for dendrogram construction 
d_rank <-read.delim('Distance_Matrix_Combined.txt',header=FALSE, sep = "\t", row.names = 1)
