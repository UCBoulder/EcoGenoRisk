...
def ec_locator(sb_name, pathway):
    ...
    ec_bsm = pd.read_csv(pathway + '/complete_binary_matrix_5_new_zeros.txt', delimiter='\t', header=0, index_col=0)
...
def substrate_changes_synbio_v_chassis(pathway, chassis_bsm, synbio_bsm):
    ...
    # Removes any spaces
    chassis_one_column_df = chassis_one_column_df[
        chassis_one_column_df['InChI_Key'].str.contains(' ') == False]
...
individual_genome_rxns = substrate_changes_synbio_v_chassis(pathway, synbio_bsm, chassis_bsm)
